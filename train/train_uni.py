...
from ir_data_uni import get_datasets as get_datasets
import models.models_uniprocessor as models
...
# from pytorch_lightning.metrics import PSNR as psnr

class UniProcessorModel(pl.LightningModule):
    def __init__(self, opt):
        super().__init__()
        self.model = models.__dict__[opt.arch]()
        # self.model = models.__dict__['uniprocessor_tiny']()
...
def main():
    ...
    valset = datasets['val']
    print(len(valset))
    checkpoint_callback = ModelCheckpoint(dirpath=opt.ckpt_dir, save_last=True, every_n_epochs=1, save_top_k=-1)
