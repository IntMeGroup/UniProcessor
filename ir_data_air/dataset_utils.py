...
class PromptTrainDataset(Dataset):
    def __init__(self, denoise_dir, dehaze_dir, derain_dir, img_size):
        super(PromptTrainDataset, self).__init__()
        self.patch_size = img_size
        self.denoise_dir = denoise_dir
        self.dehaze_dir = dehaze_dir
        self.derain_dir = derain_dir

        self.rs_ids = []
        self.hazy_ids = []
        self.D = Degradation(self.patch_size)
        self.de_temp = 0
        self.de_type = ['denoise_15', 'denoise_25', 'denoise_50', 'derain', 'dehaze'] #self.args.de_type
        print(self.de_type)
    ...
    def _init_clean_ids(self):
        ref_file = "ir_data_air/noisy/denoise.txt"
        temp_ids = []
        temp_ids+= [id_.strip() for id_ in open(ref_file)]
        clean_ids = []
        name_list = os.listdir(self.denoise_dir)
        clean_ids += [self.denoise_dir + '/' + id_.strip().split('/')[-1] for id_ in name_list if id_.strip() in temp_ids]
    ...
    def _init_hazy_ids(self):
        temp_ids = []
        hazy = "ir_data_air/hazy/hazy_outside.txt"
        temp_ids+= [self.dehaze_dir + '/haze/OTS/' + id_.strip().split('/')[-1] for id_ in open(hazy)]
    ...
    def _init_rs_ids(self):
        temp_ids = []
        rs = "ir_data_air/rainy/rainTrain.txt"
        temp_ids+= [self.derain_dir + '/input/' + id_.strip().split('/')[-1] for id_ in open(rs)]
    ...
    def _get_gt_name(self, rainy_name):
        dir_name = rainy_name.split("input/")[0] + 'target/'
        name = rainy_name.split('/')[-1].split('-')[-1]
        gt_name = dir_name + "norain-" + name
        # gt_name = rainy_name.split("rainy")[0] + 'gt/norain-' + rainy_name.split('rain-')[-1]
        return gt_name

    def _get_nonhazy_name(self, hazy_name):
        dir_name = hazy_name.split("haze/OTS")[0] + 'clear/clear_images/'
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            # cv2.imwrite("noise.png",cv2.cvtColor(img_as_ubyte(np.clip(degrad_patch, 0, 255)/255.), cv2.COLOR_RGB2BGR))
            # cv2.imwrite("noise_clean.png",cv2.cvtColor(img_as_ubyte(np.clip(clean_patch, 0, 255)/255.), cv2.COLOR_RGB2BGR))
        else:
            if de_id == 3:
                # Rain Streak Removal
                ...
                # cv2.imwrite("rain.png",cv2.cvtColor(img_as_ubyte(np.clip(degrad_img, 0, 255)/255.), cv2.COLOR_RGB2BGR))
                # cv2.imwrite("rain_clean.png",cv2.cvtColor(img_as_ubyte(np.clip(clean_img, 0, 255)/255.), cv2.COLOR_RGB2BGR))
                ...
                clean_img = crop_img(np.array(Image.open(clean_name).convert('RGB')), base=16)
                text_s = "This is a low-quality image with " + "haze" + " distortion."
                # cv2.imwrite("haze.png",cv2.cvtColor(img_as_ubyte(np.clip(degrad_img, 0, 255)/255.), cv2.COLOR_RGB2BGR))
                # cv2.imwrite("haze_clean.png",cv2.cvtColor(img_as_ubyte(np.clip(clean_img, 0, 255)/255.), cv2.COLOR_RGB2BGR))

            degrad_patch, clean_patch = random_augmentation(*self._crop_patch(degrad_img, clean_img))
        
        inp_img_blip = degrad_patch.copy()
        ...
        inp_img_blip = Image.fromarray(np.clip(np.uint8(inp_img_blip), 0, 255))
        ...
        img_name = os.path.basename(sample["clean_id"])
        data = {
            'input': degrad_patch,
            'gt': clean_patch,
            'img_name': img_name,
...
class DenoiseTestDataset(Dataset):
    ...
    def _init_clean_ids(self):
        ...
        self.clean_ids += [self.path + '/' + id_ for id_ in name_list]
    ...
    def __getitem__(self, clean_id):
        ...
        img_name = os.path.basename(self.clean_ids[clean_id])
        data = {
            'input': noisy_img,
            'gt': clean_img,
            'img_name': img_name,
...
class DerainDehazeDataset(Dataset):
    def __init__(self, path, task="derain",addnoise = False,sigma = None):
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:
            self.ids = []
            name_list = os.listdir(self.path + '/input/')
            # print(name_list)
            print(self.path)
            self.ids += [self.path + '/input/' + id_ for id_ in name_list]
    ...
    def __getitem__(self, idx):
        ...
        inp_img_blip = degraded_img.copy()
        ...
        img_name = os.path.basename(self.ids[idx])
        data = {
            'input': degraded_img,
            'gt': clean_img,
            'img_name': img_name,
